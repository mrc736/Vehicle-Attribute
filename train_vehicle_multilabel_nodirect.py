...
class Classifier(torch.nn.Module):
    ...
    def __init__(self, num_cls, input_size, is_freeze=True):
        ...
        print(self.features,'------------')
        #使用1*1的Conv2d替换全连接层Linear
        # self.fc=torch.nn.Conv2d(512*512,num_cls,(3,3),stride=(1,1))
...
class Manager(object):
    ...
    def __init__(self, options, path):
        ...
        self.loss_func = FL.focal_loss().to(device)
        # Solver
        if self.options['is_freeze']:
            ...
            #                               momentum=0.9,
            #                               weight_decay=self.options['weight_decay'])
            self.solver = torch.optim.Adam(self.net.parameters(),
                                          lr=self.options['base_lr'],
                                          momentum=0.9,
                                          weight_decay=self.options['weight_decay'])
        ...
        self.train_loader = torch.utils.data.DataLoader(self.train_set,
                                                        ...
                                                        shuffle=True,
                                                        num_workers=4,
                                                        pin_memory=True) #数据集,batchsize 默认为1
    ...
    def train(self):
        ...
        for t in range(self.options['epochs']):  # traverse each epoch
            ...
            for data, label, _ in self.train_loader:  # traverse each batch in the epoch
                ...
                loss = loss_color +2.0 * loss_type  # greater weight to type
    ...
    def test_accuracy(self, data_loader, is_draw=False):
        ...
        for data, label, f_name in data_loader:
            ...
            if is_draw:
                ...
                plt.imshow(img)
                plt.title(color_name + + ' ' + type_name)
                plt.show()
    ...
    def get_predict(self, output):
        ...
        pred = torch.cat((color_idx, type_idx), dim=1)
        return pred
    ...
    def count_correct(self, pred, label):
        ...
        for one, two in zip(pred, label):
            ...
            two=torch.index_select(two,0,indices)
            print(one,two)
    ...
    def statistics_result(self, pred, label, f_name):
        ...
        for name, one, two in zip(f_name, pred, label):
            ...
            if torch.equal(one, two):  # statistics of correct number
                ...
                err_result = label_color + ' ' + ' ' + label_type + \
                    ' => ' + \
                    pred_color + ' ' + ' ' + pred_type
...
def run():
    ...
    parser.add_argument('--is-resume',
                        type=bool,
                        default=True,
                        help='whether to resume from checkpoints')#设置为True，从Checkpoint中恢复训练
    ...
    if is_remote:  # local paths
        path = {
            ...
            'model_id': '-1',  # -1
            'train_data': 'vehicle_train_all',
            'test_data': 'vehicle_test_all'
        }
    else:  # remote paths
        path = {
            'net': './checkpoints',
